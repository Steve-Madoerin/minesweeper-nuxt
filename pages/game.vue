<template>
  ...
  async function undoField() {
    ...
    const response = await fetch('https://holy-nonah-steve-madoerin-fbd92927.koyeb.app/undo')
    if (!response.ok) {
  ...
  async function redoField() {
    ...
    const response = await fetch('https://holy-nonah-steve-madoerin-fbd92927.koyeb.app/redo')
    if (!response.ok) {
      throw new Error('Network response was not ok')
    }
    const data = await response.json()
  ...
  async function cheat() {
    ...
    console.log('cheat')
    const response = await fetch('https://holy-nonah-steve-madoerin-fbd92927.koyeb.app/cheat')
    if (!response.ok) {
      throw new Error('Network response was not ok')
    }
    const data = await response.json()
  ...
  async function informServer(command: string) {
    try {
      ...
      if (typeof command === 'string') {
        ...
        const response = await fetch(`https://holy-nonah-steve-madoerin-fbd92927.koyeb.app/infoJson/${command}`, {
          method: 'POST',
          headers: {
  ...
  async function newGame(gameSize: string) {
    try {
      ...
      const response = await fetch(`https://holy-nonah-steve-madoerin-fbd92927.koyeb.app/lgrid/${gameSize}`)
      if (!response.ok) {
        throw new Error('Network response was not ok')
      }
  ...
  async function calculateScore(time: number) {
    try {
      const response = await fetch(`https://holy-nonah-steve-madoerin-fbd92927.koyeb.app/score/${time}`)
      if (!response.ok) {
        throw new Error('Network response was not ok')
      }
      const data = await response.json()
